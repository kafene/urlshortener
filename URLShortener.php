...
function URLShortener() {
  ...
  $shorten = function($url) use($getdb, $table, $href) {
    ...
    if($test) $url = strtolower(base_convert($test, 10, 36));
    else {
  ...
  $redirect = function($id) use($getdb, $table) {
    if(preg_match('/[^a-z0-9]/', $id)) die('Invalid ID');
    $db = $getdb();
  ...
  if(!empty($_POST['url']))   $shorten($_POST['url']);
  elseif(!empty($_GET['go'])) $redirect($_GET['go']);
  else echo '<form method="post" action="" id="shortener">'
